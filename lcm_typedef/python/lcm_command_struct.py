"""LCM type definitions
This file automatically generated by lcm.
DO NOT MODIFY BY HAND!!!!
"""


from io import BytesIO
import struct

class lcm_command_struct(object):

    __slots__ = ["robot_fsm", "x_vel_des", "y_vel_des", "yaw_vel_des", "stop", "rpy_des"]

    __typenames__ = ["int8_t", "float", "float", "float", "float", "float"]

    __dimensions__ = [None, None, None, None, None, [3]]

    def __init__(self):
        self.robot_fsm = 0
        """ LCM Type: int8_t """
        self.x_vel_des = 0.0
        """
        choose state machine
        LCM Type: float
        """

        self.y_vel_des = 0.0
        """ LCM Type: float """
        self.yaw_vel_des = 0.0
        """ LCM Type: float """
        self.stop = 0.0
        """ LCM Type: float """
        self.rpy_des = [ 0.0 for dim0 in range(3) ]
        """ LCM Type: float[3] """

    def encode(self):
        buf = BytesIO()
        buf.write(lcm_command_struct._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf):
        buf.write(struct.pack(">bffff", self.robot_fsm, self.x_vel_des, self.y_vel_des, self.yaw_vel_des, self.stop))
        buf.write(struct.pack('>3f', *self.rpy_des[:3]))

    @staticmethod
    def decode(data: bytes):
        if hasattr(data, 'read'):
            buf = data
        else:
            buf = BytesIO(data)
        if buf.read(8) != lcm_command_struct._get_packed_fingerprint():
            raise ValueError("Decode error")
        return lcm_command_struct._decode_one(buf)

    @staticmethod
    def _decode_one(buf):
        self = lcm_command_struct()
        self.robot_fsm, self.x_vel_des, self.y_vel_des, self.yaw_vel_des, self.stop = struct.unpack(">bffff", buf.read(17))
        self.rpy_des = struct.unpack('>3f', buf.read(12))
        return self

    @staticmethod
    def _get_hash_recursive(parents):
        if lcm_command_struct in parents: return 0
        tmphash = (0x8113da7dab9845f3) & 0xffffffffffffffff
        tmphash  = (((tmphash<<1)&0xffffffffffffffff) + (tmphash>>63)) & 0xffffffffffffffff
        return tmphash
    _packed_fingerprint = None

    @staticmethod
    def _get_packed_fingerprint():
        if lcm_command_struct._packed_fingerprint is None:
            lcm_command_struct._packed_fingerprint = struct.pack(">Q", lcm_command_struct._get_hash_recursive([]))
        return lcm_command_struct._packed_fingerprint

    def get_hash(self):
        """Get the LCM hash of the struct"""
        return struct.unpack(">Q", lcm_command_struct._get_packed_fingerprint())[0]

